...
import com.org.scarlet.mockk.commons.*
import io.mockk.*
...
//        mockedPath.fileName()   // throws as the method returns Boolean
    ...
    fun `MockK - returns and other variants for void method`() {
        ...
        every { mockedPath.writeText(any()) } answers { Unit }
...
//        every { mockedPath.readText() } returns "successful read" andThenThrows RuntimeException()
     ...
     * anyObject, anyShort, and anyString. In MockK, these variations are all
     * replaced by a single `any` matcher.
     */
    ...
    fun `MockK - Inline Assertion`() {
        val mockPhone = mockk<Phone> {
            justRun { call(ofType(Person::class)) }
//            justRun { call(any()) }
        }
